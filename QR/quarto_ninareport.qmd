...
## Where should research focus - decision model (Norway articles) MOVE TO CORRECT PLACE PLEASE ERLEND

We can assess the conceptual development of a research field using bibliometric network analysis (@cobo2011approach). Specifically, co-word analysis assesses the relationship between between keywords in a set of documents. It helps identify key themes, their interconnections, and how they change over time, providing valuable insights for researchers and decision-makers. 

Using the method outlined in @cobo2011approach we first extracted keywords from all the included articles from Norway. We then created a Co-occurrence Matrix counting how often each pair of keywords co-occurs in the documents. The Co-occurrence Matrix was then converted into a network graph where keywords are nodes connected by edges that represent their co-occurrence.

Using the walktrap algorithm keywords are grouped into clusters. The Walktrap algorithm is a community detection method that uses random walks to explore the network and identify clusters of nodes that are more densely connected to each other than to the rest of the network. By simulating random walks and merging nodes based on their co-occurrence frequencies, it effectively reveals the community structure within the network. This makes it particularly useful for analyzing complex networks.

Each cluster represents a thematic area within the research field (where keywords within the same cluster co-occur more often). The Centrality of each cluster indicates the importance of the cluster in the entire network. High Centrality suggests that themes are well-integrated within the research field. They act as bridges between different topics and are often considered core or mainstream themes. Themes with low centrality are more isolated and have fewer connections to other themes. These could be emerging topics or niche areas that are not yet widely integrated into the broader research field.

Density measures the internal cohesion of a theme. It indicates how strongly the keywords within a particular theme are connected to each other. It reflects the extent to which a theme is self-contained or focused. Themes with high density have strong internal connections, meaning the keywords within the theme frequently co-occur with each other. These themes are well-developed and internally cohesive, often representing specialised areas of research. Themes with low density have weaker internal connections. The keywords within these themes do not co-occur as frequently, indicating that the theme might be broader, less focused, or still developing.

We plotted the network in a "strategic diagram" where themes are plotted based on their centrality and density, and categorised by their position in the quadrants. 

- Motor Themes (High Centrality, High Density): These are important and well-developed themes that are central to the research field. They have strong internal cohesion and are widely connected to other themes.

- Basic and Transversal Themes (High Centrality, Low Density): These themes are important but less cohesive. They serve as bridges connecting various parts of the research field but may encompass a broader range of topics.

- Niche Themes (Low Centrality, High Density): These themes are specialised and internally cohesive but not well-integrated into the broader research field. They represent focused areas of research.

- Emerging or Declining Themes (Low Centrality, Low Density): These themes are less developed. They could be new and emerging topics or older themes that are declining in relevance.
...
In @fig-themeMap themes are indicated with a single common keyword. We can see that well studied topics within Norway  included species richness particularly in the context of forest management and reindeer in the context of wild reindeer, power infrastructure as well as domestic reindeer herding. Studies around habitat selection included studies on lynx and roe deer. Human-wildlife conflict around Geese and agriculture is also well studied.  

Topics that are emerging include hyttebygging (recreational infrastructure) and biodiversity conservation in conjunction with climate change as well as sustainable development. Future scenarios of land use change (as well as other global change) is also an emerging field. Studies around indigenous peoples within the context of landuse change are under-represented in the field. 

# Diskusjon

<!--# Erlend can you improve this and move to correct place (or delete!).. it looks really long when pasted here! -->

Forskjellige studiedesign tilbyr varierende nivåer av strenghet og pålitelighet i å informere beslutningsmodeller. Randomiserte kontrollerte studier (RCT), ansett som gullstandarden i forskningsmetodologi, gir det høyeste bevisnivået på grunn av deres randomiserte tildeling av deltakere til intervensjons- og kontrollgrupper. Ved å minimere skjevheter og forvirrende faktorer, gir RCTer robust bevis for årsakssammenhenger mellom intervensjoner og utfall. Dette gjør dem spesielt verdifulle for beslutningsmodeller som søker å nøyaktig forutsi effektene av intervensjoner på en målpopulasjon.

I motsetning til dette mangler observasjonsstudier, som før-etter-kontroll-påvirkning (BACI), kontroll-påvirkning (CA), før-etter (BA) og etter (A) studier, randomiseringen som er iboende i RCTer. Selv om disse designene fortsatt kan gi verdifull innsikt i sammenhenger mellom intervensjoner og utfall, er de mer utsatt for skjevheter og forvirrende variabler. For eksempel, uten randomisering, kan det være utfordrende å tilskrive observerte endringer utelukkende til intervensjonen, noe som kan føre til potensial overestimering eller underestimering av intervensjonseffekter. Følgelig kan beslutninger basert på observasjonsstudier ha større usikkerhet og være mindre pålitelige for å informere politikk eller praksis sammenlignet med de som er basert på RCTer.

Valget av studiedesign avhenger av nivået av bevis som kreves og gjennomførbarheten av å utføre randomiserte studier i den gitte konteksten. Mens RCTer tilbyr det høyeste nivået av tillit til årsaksslutning, kan observasjonsstudier fortsatt gi verdifulle innsikter, dog med større forsiktighet på grunn av deres mottakelighet for skjevheter.
