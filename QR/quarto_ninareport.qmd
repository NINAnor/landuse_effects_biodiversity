...
Litteratursøket resulterte i totalt `r params$'search-results'` artikler (@fig-prisma). På sammendragsscreeningstadiet screenet vi 16 093 artikler, og ekskluderte 13 472 av dem. På fulltekststadiet vurderte vi 2 586 for inkludering, og endte opp med 598 artikler i gjennomgangen.p

![Flytskjema (PRISMA) som viser hvor mange artikler som går videre til de ulike stegene i den systematiske prosessen for litterturgjennomgang.](images/prisma%20(3).png){#fig-prisma}
...
Vi benyttet i denne gjennomgangen klassifiseringssystemet essensielle biodiversitetsvariabler (EBV) til å kategorisere hvilken biologisk responsvariabel som ble studert. EBV er et hierarkisk system som består av seks klasser og 21 variabler. Av de seks klassene i @fig-EBV-class er det flest studier som har fokusert på arters bestander (*Species populations*) samfunnssammensetning (*Community composition*) og arters egenskaper (*Species traits*). Artsegenskaper inkluderer for eksempel artenes reproduksjon, atferd, diett og bevegelse i landskapet osv. I kontrast til dette var det svært få studier som studerte hele økosystemers funksjon (*Ecosystem functioning*) eller struktur (*Ecosystem structure*). Tilsvarende var det veldig få som studerte effekter av arealbruk på genetisk sammensetning (*Genetic composition*).
...
Den hyppigst forekommende økosystemtjenesten i vårt datamateriale for studier som har benyttet naturvitenskapelige metoder var lagring og sekvestrering av karbon (*R & M Carbon storage and sequestraton*). Det var et mindre antall studier som studerte kulturelle økosystemtjenester (@fig-ES)
...
Her følger resultatene fra vår gjennomgang av de studiene som har analysert en eller annen form for konflikt, et forvaltningsverktøy eller styring mer generelt (*governance*). Dette tilsvarer `r nrow(join_Nor_form_soc)` artikler av total `r nrow(join_Nor_form)` artikler (`r round(nrow(join_Nor_form_soc)/nrow(join_Nor_form)*100)`%). Av disse igjen er det kun `r nrow(join_Nor_form_soc |> filter(study_area_discipline == "Social Sciences"))` artikler som bruker samfunsvitenskapelige metoder, det vil si `r round(((nrow(join_Nor_form_soc |> filter(study_area_discipline == "Social Sciences")) / nrow(join_Nor_form)) * 100),0)`% av studiene totalt.
...
Hovedparten av de samfunnsrelaterte studiene hadde en analytisk tilnærming som omhandlet styring og forvaltning (@fig-analytisk). Deretter følger tilnærminger som vektlegger økonomiske forhold, aktørers oppfatninger og forståelser, deltagende, demokratiske og/eller deliberative prosesser, samt maktrelasjoner og aktørgrupper. Færre studier vektlegger analytiske tilnærminger knyttet til verdier, kunnskap, bærekraft, tverrsektorielle prosesser og gjennomgripende endringer (transformasjoner)[.]{.underline}
...
Av den samfunnsrelaterte og den samfunnsvitenskapelige forskningen vi har undersøkt er flest studier relatert til skogøkosystemer, etterfulgt av semi-naturlige økosystemer og fjelløkosystemer   (@fig-socialscience-ecosys).
...
Hovedvekten av de systematiske litteratursammenstillingene har fokusert på arealbrukskategoriene jordbruk (*Agriculture*), inkludert beiting, og bruk av biologiske ressur (skogbruk, jakt, fisk osv.; *Biological resource use*) (@fig-sysrev-areause-coarse; @fig-sysrev-areause-detailed).
...
De systematiske litteratursammenstillingene som forekommer i datamaterialet har betydelige risiko for systematisk skjevhet og har i mindre grad vurdert denne skjevheten i de inindividuelle studiene som de har benyttet seg av (@fig-riskOfBias).
